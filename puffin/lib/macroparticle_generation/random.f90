...
public :: init_random_seed, random_normal, random_Poisson
...
REAL(KIND=WP) , INTENT(IN)    :: aa, bb
LOGICAL, INTENT(IN) :: first
REAL(KIND=WP)                 :: fn_val

!     Local variables
REAL(KIND=WP) , PARAMETER  :: aln4 = 1.3862944
REAL(KIND=WP)              :: a, b, g, r, s, x, y, z
REAL(KIND=WP) , SAVE       :: d, f, h, t, c
LOGICAL, SAVE    :: swap
...
REAL(KIND=WP) , INTENT(IN)    :: h, b
LOGICAL, INTENT(IN) :: first
REAL(KIND=WP)                 :: fn_val

!     Local variables
REAL(KIND=WP)             :: ym, xm, r, w, r1, r2, x
REAL(KIND=WP) , SAVE      :: a, c, d, e
...
!     ..
!     .. Local Scalars ..
REAL(KIND=WP)           :: b1, b2, c, c0, c1, c2, c3, del, difmuk, e, fk, fx, fy, g,  &
                 omega, px, py, t, u, v, x, xx
REAL(KIND=WP) , SAVE    :: s, d, p, q, p0
INTEGER(KIND=IP)        :: j, k, kflag
LOGICAL, SAVE :: full_init
INTEGER(KIND=IP) , SAVE :: l, m
!     ..
!     .. Local Arrays ..
REAL(KIND=WP) , SAVE    :: pp(35)
!     ..
!     .. Data statements ..
REAL(KIND=WP) , PARAMETER :: a0 = -.5, a1 = .3333333, a2 = -.2500068, a3 = .2000118,  &
...
INTEGER(KIND=IP)          :: ru, rd
INTEGER(KIND=IP) , SAVE   :: r0
REAL(KIND=WP)             :: u, pd, pu
REAL(KIND=WP) , SAVE      :: odds_ratio, p_r
REAL(KIND=WP) , PARAMETER :: zero = 0.0, one = 1.0
...
!     ..
!     .. Local Scalars ..
REAL(KIND=WP)             :: alv, amaxp, f, f1, f2, u, v, w, w2, x, x1, x2, ynorm, z, z2
REAL(KIND=WP) , PARAMETER :: zero = 0.0, half = 0.5, one = 1.0
INTEGER(KIND=IP)          :: i, ix, ix1, k, mp
INTEGER(KIND=IP) , SAVE   :: m
REAL(KIND=WP) , SAVE      :: p, q, xnp, ffm, fm, xnpq, p1, xm, xl, xr, c, al, xll,  &
                   xlr, p2, p3, p4, qn, r, g
...
REAL(KIND=WP) , INTENT(IN)   :: sk, p
INTEGER(KIND=IP)             :: ival

!     Local variables
! THE PARAMETER ULN = -LOG(MACHINE'S SMALLEST REAL NUMBER).

REAL(KIND=WP) , PARAMETER    :: h = 0.7
REAL(KIND=WP)                :: q, x, st, uln, v, r, s, y, g
...
INTEGER(KIND=IP)           :: j, n
INTEGER(KIND=IP) , SAVE    :: nk
REAL(KIND=WP) , PARAMETER  :: pi = 3.14159265
REAL(KIND=WP) , SAVE       :: p(20), theta(0:20)
REAL(KIND=WP)              :: sump, r, th, lambda, rlast
REAL(KIND=WP)         :: dk
...
INTEGER(KIND=IP) , INTENT(IN)  :: n
INTEGER(KIND=IP) , INTENT(OUT) :: order(n)
...
INTEGER(KIND=IP) , INTENT(IN)  :: iounit
